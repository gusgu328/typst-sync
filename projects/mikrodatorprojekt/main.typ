...
I figur 2 visas ett blockschema över de komponenter som används i projektet, och övergripande bild på deras interna kommunikation med varandra.  I nästa avsnitt beskrivs varje komponent för sig.
...
I detta projekt har det använts en LCD-display HD4480, en OLED-display ssd1309, en ATmega16A processor, en ljud-enhet i form av en piezoelektrisk högtalare samt två tryckknappar. Dessa komponenter är monterade på ett DAvid-kort. I denna del av rapporten kommer vi att gå igenom dessa olika delar och förklara hårdvarans funktioner och hur de användes i projektet.
...
Atmega16A är hjärnan på DAvid-kortet och styr alla ingångs- och utgångs-komponenter, via antingen sina I/O-pinnar eller TWI (IC2). Den är utrustad med ett 16 kb flashminne för lagring av programkod, 1 kb SRAM för variabelhantering under körning samt 512 byte EEPROM för permanent lagring på processorn.
...
TWI-bussen använder endast två ledare SDA (data) och SCL (klockan). SDA används för att skicka och ta emot data. Det är själva överföringen av data och är i vilande tillstånd hög. SCL är klocksignaler som masterenheten genererar. Dessa signaler styr tempot i dataöverföringen, där endast data får ändras vid fallande flank på SCL och läses av vid stigande flank.

En transaktion på TWI-bussen inleds alltid av masterenheten. Mastern skickar en startsignal vilket innebär att SDA går lågt medan SCL fortfarande är hög. Därefter skickar master-enheten en 7-bitars adress som motsvarar en av slavenheternas adress, följt av en R/W-bit. Därefter kommer en ack-bit som visar att denna del av transaktionen är klar. Därefter kommer dataöverföringen, där beroende på R/W skickas data eller tas de emot data. Data skickas och tas emot med en sekvens av en byte, där varje byte följs av en ack-bit. När hela transaktionen sedan är färdig skickar mastern en stoppsignal med hjälp av SDA och SCL, vilket frigör TWI-bussen för nya transaktioner.

== LCD HD4480 (textdisplay)

Denna display är en LCD-display vilket betyder att det är en “Liquid Crystal Display”, vilket betyder att den har ett lager av flytande kristaller som kan ändra hur ljus passerar genom dem med hjälp av elektrisk spänning, så pixlar blir ljus/ mörka.
